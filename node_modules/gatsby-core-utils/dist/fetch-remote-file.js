  ...
  if (args.cacheKey) {
    ...
    if ((info === null || info === void 0 ? void 0 : info.cacheKey) === args.cacheKey && fileDirectory) {
      ...
      if (await _fsExtra.default.pathExists(cachedPath)) {
        ...
        if (!cachedPath.startsWith(_path.default.join((_global$__GATSBY$root = (_global$__GATSBY = global.__GATSBY) === null || _global$__GATSBY === void 0 ? void 0 : _global$__GATSBY.root) !== null && _global$__GATSBY$root !== void 0 ? _global$__GATSBY$root : process.cwd(), `public`))) {
          return cachedPath;
...
async function fetchFile({
  ...
  try {
    ...
    const cachedEntry = await storage.remoteFileInfo.get(url); // Add htaccess authentication if passed in. This isn't particularly
    ...
    await _fsExtra.default.ensureDir(_path.default.join(fileDirectory, digest));
    const tmpFilename = (0, _filenameUtils.createFilePath)(fileDirectory, `tmp-${digest}`, ext);
    const filename = (0, _filenameUtils.createFilePath)(_path.default.join(fileDirectory, digest), name, ext); // See if there's response headers for this url
    // from a previous request.

    const headers = { ...httpHeaders
    };

    if (cachedEntry !== null && cachedEntry !== void 0 && (_cachedEntry$headers = cachedEntry.headers) !== null && _cachedEntry$headers !== void 0 && _cachedEntry$headers.etag && (await (0, _fsExtra.pathExists)(filename))) {
    ...
    if (response.statusCode === 200) {
      ...
      await setInFlightObject(url, BUILD_ID, {
        ...
        directory: (0, _path2.slash)(fileDirectory),
        path: (0, _path2.slash)(filename.replace(fileDirectory, ``))
      });
